...
class TriviaCog(commands.Cog):
    ...
    async def fetch_question(self, category=None):
        ...
        try:
            ...
            logger.error(f"Unexpected error fetching question: {str(e)}")
            return None
    ...
    async def trivia(self, ctx):
        ...
        try:
            ...
            await ctx.send("An error occurred while processing the trivia question. Please try again later.")
    ...
    async def trivia_game(self, ctx):
        ...
        try:
            ...
            if len(players) > 0:
                await asyncio.sleep(30)  # Give additional time for more players to join after the first one
            ...
            logger.error(f"Error in trivia game: {str(e)}")
            await ctx.send("An error occurred during the trivia game. The game has been stopped.")
            if ctx.channel.id in self.active_games:
                del self.active_games[ctx.channel.id]
    ...
    async def stop_game(self, ctx):
        ...
        try:
            ...
            await ctx.send("An error occurred while trying to stop the game.")
